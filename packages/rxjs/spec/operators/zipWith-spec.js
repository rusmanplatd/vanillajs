...
describe('zipWith', function () {
  ...
  it('should end once one observable completes and its buffer is empty', function () {
    rxTestScheduler.run(function (_a) {
      ...
      expectObservable(e1.pipe(operators_1.zipWith(e2, e3))).toBe(
        expected,
        values
      );
  ...
  describe('with iterables', function () {
    it('should zip them with values', function () {
      rxTestScheduler.run(function (_a) {
        ...
        expectObservable(e1.pipe(operators_1.zipWith(myIterator))).toBe(
          expected,
          values
        );
        expectSubscriptions(e1.subscriptions).toBe(e1subs);
  ...
  it('should work with n-ary symmetric', function () {
    rxTestScheduler.run(function (_a) {
      ...
      var c = hot('---1-^---3---6-|  ');
      var expected = '  ----x---y-|  ';
      expectObservable(a.pipe(operators_1.zipWith(b, c))).toBe(expected, {
        x: ['1', '2', '3'],
        y: ['4', '5', '6'],
      });
      expectSubscriptions(a.subscriptions).toBe(asubs);
  ...
  it('should work with error and error', function () {
    rxTestScheduler.run(function (_a) {
      ...
      var bsubs = '   ^-----!';
      var expected = '------#';
      expectObservable(a.pipe(operators_1.zipWith(b))).toBe(
        expected,
        null,
        'too bad'
      );

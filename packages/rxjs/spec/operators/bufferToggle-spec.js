...
describe('bufferToggle operator', function () {
  ...
  it('should emit buffers using hot openings and hot closings', function () {
    testScheduler.run(function (_a) {
      ...
      var result = e1.pipe(
        operators_1.bufferToggle(e2, function (x) {
          return e3;
        })
      );
  ...
  it('should emit buffers using varying cold closings', function () {
    testScheduler.run(function (_a) {
      ...
      var i = 0;
      var result = e1.pipe(
        operators_1.bufferToggle(e2, function () {
          return closings[i++];
        })
      );
      expectObservable(result).toBe(expected, values);
  ...
  it('should accept openings resolved promise', function (done) {
    var e1 = rxjs_1.concat(
      rxjs_1.timer(10).pipe(operators_1.mapTo(1)),
      rxjs_1.timer(100).pipe(operators_1.mapTo(2)),
      rxjs_1.timer(150).pipe(operators_1.mapTo(3)),
      rxjs_1.timer(200).pipe(operators_1.mapTo(4))
    );
    var expected = [[1]];
    e1.pipe(
      operators_1.bufferToggle(
  ...
  it('should accept closing selector that returns a resolved promise', function (done) {
    ...
    var expected = [[1]];
    e1.pipe(
      operators_1.bufferToggle(rxjs_1.of(10), function () {

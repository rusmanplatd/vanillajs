...
describe('zip', function () {
  ...
  describe('with iterables', function () {
    ...
    it('should work with non-empty observable and non-empty iterable selector that throws', function () {
      rxTestScheduler.run(function (_a) {
        ...
        expectObservable(rxjs_1.zip(a, b, selector)).toBe(
          expected,
          { x: '14' },
          new Error('too bad')
        );
  ...
  it('should work with n-ary symmetric', function () {
    rxTestScheduler.run(function (_a) {
      ...
      var c = hot('---1-^---3---6-|  ');
      var expected = '  ----x---y-|  ';
      expectObservable(rxjs_1.zip(a, b, c)).toBe(expected, {
        x: ['1', '2', '3'],
        y: ['4', '5', '6'],
      });
      expectSubscriptions(a.subscriptions).toBe(asubs);
  ...
  it('should work with n-ary symmetric selector', function () {
    rxTestScheduler.run(function (_a) {
      ...
      var c = hot('---1-^---3---6-|  ');
      var expected = '  ----x---y-|  ';
      var observable = rxjs_1.zip(a, b, c, function (r0, r1, r2) {
        return [r0, r1, r2];
      });
  ...
  it('should work with two sources that eventually raise errors', function () {
    rxTestScheduler.run(function (_a) {
      ...
      var bsubs = '   ^-------!    ';
      var expected = '-----x--#    ';
      expectObservable(rxjs_1.zip(a, b)).toBe(
        expected,
        { x: [1, 2] },
        'too bad'
      );
